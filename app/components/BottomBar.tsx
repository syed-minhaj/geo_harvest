...
export default function BottomBar() {
    ...
    return (
        <div className="sm:hidden flex flex-row w-full h-16 px-4 bg-card dark:bg-bg2 sticky bottom-0 justify-between items-center border-t border-border">
            {Pages.map((page, index) => (
                <Link href={page === "dashboard" ? "/app" : `/app/${page}`} key={index} 
                    className={`  rounded-r1 border flex flex-col items-center text-sm 
                    hover:bg-bg1 hover:border-border 
